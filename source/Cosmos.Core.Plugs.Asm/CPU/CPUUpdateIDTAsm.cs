...
namespace Cosmos.Core.Plugs.Asm.Assemblers.CPU
{
    ...
    public class CPUUpdateIDTAsm : CPUAll.AssemblerMethod
    {
        ...
        {
            return GetMethodDef(Assembly.ReflectionOnlyLoadFrom("Cosmos.Core.INTs"), Type.GetType("Cosmos.Core.INTs").FullName, "HandleInterrupt_" + aInterrupt.ToString("X2"), false);
        }
        ...
        public override void AssembleNew(Cosmos.Assembler.Assembler aAssembler, object aMethodInfo)
        {
            ...
            for (int j = 0; j < 256; j++)
            {
                ...
                if (xHandler == null)
                {
                    xHandler = GetMethodDef(Assembly.ReflectionOnlyLoadFrom("Cosmos.Core.INTs"), Type.GetType("Cosmos.Core.INTs").FullName, "HandleInterrupt_Default", true);
                }

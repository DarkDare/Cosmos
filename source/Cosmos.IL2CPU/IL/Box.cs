...
namespace Cosmos.IL2CPU.X86.IL
{
  ...
  public class Box : ILOp
  {
    ...
    public override void Execute(MethodInfo aMethod, ILOpCode aOpCode)
    {
      ...
      XS.Push(ObjectInfo.FieldDataOffset + xSize);
      XS.Call(LabelName.Get(GCImplementationRefs.AllocNewObjectRef));
      XS.Pop(ESI);
      XS.Set(EBX, xTypeID, sourceIsIndirect: true);
      XS.Set(ESI, EBX, destinationIsIndirect: true);
      XS.Set(ESI, (uint)ObjectInfo.InstanceTypeEnum.BoxedValueType, destinationDisplacement: 4, size: RegisterSize.Int32);
      new Comment(Assembler, "xSize is " + xSize);
      for (int i = 0; i < (xSize / 4); i++)
      {
        XS.Pop(EDX);
        XS.Set(ESI, EDX, destinationDisplacement: (ObjectInfo.FieldDataOffset + (i * 4)), size: RegisterSize.Int32);
      }
      XS.Push(ESI);
      XS.Push(0);
    }
